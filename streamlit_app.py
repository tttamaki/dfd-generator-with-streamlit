...
def generate_dfd():
    ...
    if dfd_text:
        ...
        process, output_file_path = run_data_flow_diagram(
            input_file_path, st.session_state.output_format)

        # Generate the PDF format
        process_pdf, pdf_file_path = run_data_flow_diagram(
            input_file_path, "pdf")

        process_dfd_results(process, process_pdf,
                            output_file_path, pdf_file_path, dfd_text)
...
def initialize_dfd_text():
    ...
    if "dfd_text" not in st.session_state:
        ...
        if encoded_text:
            try:
                decoded_text = bz2.decompress(
                    bytes.fromhex(encoded_text)).decode()
...
def display_generated_dfd():
    ...
    if st.session_state.get("generated", False):
        ...
        st.text_input(
            "Copy the generated DFD URL:",
            st.session_state.generated_url, help="Copy this URL manually")
